...
def test_wilson_confidence_is_interval_concentration():
    ...
    assert 0 < perf.acceptance_lower_bound < perf.acceptance_upper_bound <= 1
    assert abs(perf.confidence-(1-(perf.acceptance_upper_bound-perf.acceptance_lower_bound))) < 1e-12
...
def test_adaptive_router_falls_back_when_evidence_is_insufficient():
    ...
    result=AdaptiveModelRouter(ledger).route(profiles(),ModelRouteRequest("t",["code"],min_strength=.5,optimize="cost",metadata={"task_class":"backend","min_empirical_samples":3}))
    assert result.adaptive is False and result.selected_model_id=="luna"
...
def test_adaptive_cost_per_quality_prefers_measured_cheaper_success():
    ...
    result=AdaptiveModelRouter(ledger).route(profiles(),ModelRouteRequest("job",["code"],min_strength=.5,metadata={"task_class":"backend","min_empirical_samples":3,"empirical_optimize":"cost_per_quality"}))
    assert result.adaptive is True and result.selected_model_id=="luna"
    assert result.static_selected_model_id is not None and result.performance["luna"]["samples"]==8
...
def test_empirical_floor_never_overrides_static_strength_floor():
    ...
    result=AdaptiveModelRouter(ledger).route(profiles(),ModelRouteRequest("arch",["code"],min_strength=.9,metadata={"task_class":"architecture","min_empirical_samples":3}))
...
def test_deterministic_calibration_routes_under_sampled_eligible_model():
    ...
    result=AdaptiveModelRouter(ledger).route(profiles(),ModelRouteRequest("cal",["code"],min_strength=.5,metadata={"task_class":"tests","min_empirical_samples":2,"explore_under_sampled":True}))
    assert result.adaptive is True and result.reason.startswith("deterministic calibration") and result.selected_model_id=="terra"
