...
def test_cli_exposes_durable_commands():
    ...
    for argv, command in [
        ...
        (["fleet-control", "machine_x", "--store", "x.db", "--policy", "fleet.json"], "fleet-control"),
        (["fleet-refresh", "machine_x", "--store", "x.db", "--policy", "collaboration.json"], "fleet-refresh"),
        (["telemetry", "machine_x", "--store", "x.db"], "telemetry"),
        (["telemetry", "machine_x", "--store", "x.db", "--policy", "telemetry.json"], "telemetry"),
        ...
        (["worker-control", "machine_x", "--store", "x.db", "--worker", "w", "--action", "DRAIN", "--actor", "operator", "--reason", "maintenance"], "worker-control"),
        (["artifacts", "machine_x", "--store", "x.db", "--limit", "20"], "artifacts"),
