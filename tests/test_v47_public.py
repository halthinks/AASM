...
def test_v47_public_contract_is_current_and_composes_over_v46():
    ...
    assert AASMEngine is V47Engine
    assert issubclass(V47Engine, V46Engine)
    report = validate_public_api_contract()
    assert report["valid"], report
    contract = report["contract"]
    assert contract["contract_version"] == "0.23.0"
    assert contract["runtime_version"] == "0.47.0"
    assert contract["distribution"]["version"] == "0.47.0"
