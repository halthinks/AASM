...
def test_trace_contract_and_version_are_public():
    report=validate_public_api_contract(); assert report["valid"] is True; assert report["contract"]["runtime_version"]==__version__; assert report["contract"]["contract_version"]; assert trace_contract()["contract_id"]=="aasm.trace.v1"; assert provenance_contract()["contract_id"]=="aasm.provenance.v1"; assert semantic_problem_contract()["contract_id"]=="aasm.semantic.problem.v1"; assert semantic_compiler_contract()["contract_id"]==SEMANTIC_COMPILER_CONTRACT_ID
