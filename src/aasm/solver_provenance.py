...
SOLVER_PROVENANCE_STABILITY = "FOUNDATION_EXPERIMENTAL"
...
def _options(value: Mapping[str, Any]) -> dict[str, Any]:
...
class SolverExecutionProfile:
    ...
    def __post_init__(self) -> None:
        ...
        requested = _options(self.requested_options)
        required = _options(self.required_effective_options)
        if self.determinism_policy == "STRICT_EFFECTIVE_OPTIONS" and not required:
            raise ValueError("strict effective-options profile requires required_effective_options")
        object.__setattr__(self, "requested_options", requested)
        object.__setattr__(self, "required_effective_options", required)
        for name in (
            "provider_id",
            "provider_version",
            "adapter_id",
        ...
        if bool(self.numeric_policy_id) != bool(self.numeric_policy_fingerprint):
            raise ValueError("numeric policy ID and fingerprint must be supplied together")
    ...
    def identity_payload(self) -> dict[str, Any]:
        return {
            ...
            "requested_options": _options(self.requested_options),
            "required_effective_options": _options(self.required_effective_options),
...
class SolverRuntimeProvenance:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "requested_options", _options(self.requested_options))
        object.__setattr__(self, "effective_options", _options(self.effective_options))
        for pair in (
            ("provider_status_map_id", "provider_status_map_fingerprint"),
            ("numeric_policy_id", "numeric_policy_fingerprint"),
        ):
            if bool(getattr(self, pair[0])) != bool(getattr(self, pair[1])):
                raise ValueError(f"{pair[0]} and {pair[1]} must be supplied together")
        for name in (
            "build_fingerprint",
    ...
    def identity_payload(self) -> dict[str, Any]:
        return {
            ...
            "requested_options": _options(self.requested_options),
            "effective_options": _options(self.effective_options),
...
def evaluate_solver_execution_profile(
    ...
    for field_name in ("provider_id", "provider_version"):
        ...
        if expected and getattr(actual, field_name) != expected:
            deviations.append({"code": "PROVIDER_IDENTITY_MISMATCH", "key": field_name, "expected": expected, "actual": getattr(actual, field_name)})
    ...
    return SolverProfileEvaluation(
        selected.profile_id,
        selected.fingerprint,
...
def solver_provenance_contract() -> dict[str, Any]:
    return {
        ...
        "environment": "FINGERPRINT_BOUND",
        "numeric_policy": "OPTIONALLY_FINGERPRINT_BOUND",
        "strict_profile": "REQUIRED_EFFECTIVE_OPTIONS_MUST_MATCH_EXACTLY",
