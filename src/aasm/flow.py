...
class ResourceFlowAllocator:
    ...
    def solve(self, capacities:dict[str,dict[str,float]], source:str, sink:str):
        ...
        for u,vs in capacities.items():
            for v,c in vs.items(): residual[u][v]=float(c); residual[v].setdefault(u,0.0)
        total=0.0
        while True:
        ...
        for u,vs in capacities.items():
            for v,c in vs.items():
                if u in reachable and v not in reachable and c>0: cut.append((u,v,c))
        return {"max_flow":total,"min_cut_edges":cut,"reachable":sorted(reachable)}
