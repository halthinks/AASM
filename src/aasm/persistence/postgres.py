...
class PostgresStore:
    ...
    def append(self, machine_id: str, event: Event, snapshot: MachineSnapshot) -> Event:
        ...
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                ...
                cur.execute("SELECT event_json FROM aasm_events WHERE machine_id=%s ORDER BY sequence",(machine_id,))
                existing=[event_from_dict(self._obj(row[0])) for row in cur.fetchall()]
            ...
            with self._conn.cursor() as cur:
                cur.execute(
                    """INSERT INTO aasm_checkpoints(checkpoint_id,machine_id,snapshot_json,reason,created_at)
                       ...
                       ON CONFLICT(checkpoint_id) DO UPDATE SET snapshot_json=excluded.snapshot_json,reason=excluded.reason""",
    ...
    def find_effect_by_idempotency(self,machine_id,idempotency_key):
        with self._conn.cursor() as cur:
            cur.execute("SELECT effect_json FROM aasm_effects WHERE machine_id=%s AND idempotency_key=%s",(machine_id,idempotency_key)); row=cur.fetchone()
        return None if row is None else effect_from_dict(self._obj(row[0]))

    def list_effects(self,machine_id):
        with self._conn.cursor() as cur:
            cur.execute("SELECT effect_json FROM aasm_effects WHERE machine_id=%s ORDER BY created_at,effect_id",(machine_id,)); rows=cur.fetchall()
    ...
    def claim_effect_attempt(self,machine_id,effect_id):
        """Atomically move one effect attempt into RUNNING under a row lock."""
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                cur.execute(
                    "SELECT effect_json FROM aasm_effects WHERE machine_id=%s AND effect_id=%s FOR UPDATE",
                ...
                if record.status == EffectStatus.SUCCEEDED.value:
                    return record
                cur.execute(
                    "UPDATE aasm_effects SET status=%s,effect_json=%s::jsonb,updated_at=%s WHERE machine_id=%s AND effect_id=%s",
                    (record.status,self._j(effect_to_dict(record)),record.updated_at,machine_id,effect_id),
                )
                return record
    ...
    def _check_claim_limits(active, *, worker_id, resource_id, demand, resource_capacity, quotas):
        ...
        for raw in quotas or []:
            ...
            selected=[row for row in active if scope=="machine" or (scope=="worker" and row[0]==worker_id) or (scope=="resource" and row[1]==resource_id)]
            ...
            if max_units is not None and sum(float(row[2] or 0) for row in selected)+demand > float(max_units)+1e-12: raise ValueError(f"Quota exceeded: {raw.get('quota_id')}")
    ...
    def _canonical_claim_policy(snapshot: MachineSnapshot, worker_id: str, requested_resource_id: str | None, at_time: float):
        ...
        heartbeat=float(worker.get("last_heartbeat",0) or 0); timeout=float(worker.get("heartbeat_timeout",60) or 60)
        if at_time > heartbeat + timeout: raise ValueError(f"Worker {worker_id} is stale")
        resource_id=worker.get("resource_id")
        if requested_resource_id is not None and requested_resource_id != resource_id:
            raise ValueError(f"Stale worker resource mapping: requested {requested_resource_id}, canonical is {resource_id}")
    ...
    def acquire_task_claim(self,machine_id,task_id,lease_id,worker_id,expires_at,at_time,*,resource_id=None,demand=0.0,resource_capacity=None,quotas=None):
        ...
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                ...
                cur.execute("SELECT snapshot_json FROM aasm_runs WHERE machine_id=%s FOR UPDATE",(machine_id,)); row=cur.fetchone()
                if row is None: raise KeyError(machine_id)
                canonical=snapshot_from_dict(self._obj(row[0]))
                canonical_resource,canonical_capacity,canonical_quotas=self._canonical_claim_policy(canonical,worker_id,resource_id,float(at_time))
                ...
                cur.execute("SELECT worker_id,resource_id,demand FROM aasm_task_claims WHERE machine_id=%s AND expires_at>%s",(machine_id,at_time)); active=cur.fetchall()
                ...
                cur.execute(
                    "INSERT INTO aasm_task_claims(machine_id,task_id,lease_id,worker_id,expires_at,resource_id,demand) VALUES(%s,%s,%s,%s,%s,%s,%s)",
    ...
    def renew_task_claim(self,machine_id,lease_id,expires_at):
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                cur.execute("UPDATE aasm_task_claims SET expires_at=%s WHERE machine_id=%s AND lease_id=%s",(expires_at,machine_id,lease_id)); return cur.rowcount==1
