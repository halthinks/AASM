...
class PostgresStore:
    ...
    @staticmethod
    def _event_lock_key(machine_id: str) -> str: return f"event:{machine_id}"
    @staticmethod
    def _claim_lock_key(machine_id: str) -> str: return f"claims:{machine_id}"
    ...
    def initialize_run(self, snapshot: MachineSnapshot):
        ...
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO aasm_runs(machine_id,snapshot_json,state,version,created_at,updated_at) VALUES(%s,%s::jsonb,%s,%s,%s,%s) ON CONFLICT(machine_id) DO NOTHING",
    ...
    def acquire_task_claim(self,machine_id,task_id,lease_id,worker_id,expires_at,at_time,*,resource_id=None,demand=0.0,resource_capacity=None,quotas=None):
        ...
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                cur.execute("SELECT pg_advisory_xact_lock(hashtext(%s))",(self._event_lock_key(machine_id),)); cur.execute("SELECT pg_advisory_xact_lock(hashtext(%s))",(self._claim_lock_key(machine_id),))
                cur.execute("SELECT snapshot_json FROM aasm_runs WHERE machine_id=%s FOR UPDATE",(machine_id,)); row=cur.fetchone()
                if row is None: raise KeyError(machine_id)
                canonical_resource,canonical_capacity,canonical_quotas=self._canonical_claim_policy(snapshot_from_dict(self._obj(row[0])),worker_id,resource_id,float(at_time))
                cur.execute("DELETE FROM aasm_task_claims WHERE machine_id=%s AND expires_at<=%s",(machine_id,at_time)); cur.execute("SELECT 1 FROM aasm_task_claims WHERE machine_id=%s AND task_id=%s",(machine_id,task_id))
                ...
                cur.execute("INSERT INTO aasm_task_claims(machine_id,task_id,lease_id,worker_id,expires_at,resource_id,demand) VALUES(%s,%s,%s,%s,%s,%s,%s)",(machine_id,task_id,lease_id,worker_id,expires_at,canonical_resource,float(demand))); return True
