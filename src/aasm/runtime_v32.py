...
from .semantic_result import (
    DomainPackage, ProblemDefinition, ProblemModel, ProblemInstance,
    SEMANTIC_PROBLEM_CONTRACT_ID, canonical_semantic_json,
    semantic_problem_contract, semantic_problem_document, semantic_problem_from_document,
    validate_problem_instance,
)


class AASMEngine(V31Engine):
    """v0.35 runtime: distributed safety plus event-sourced semantic problem binding."""
    ...
    def _finish_lease(self, lease_id: str, status: str, *, result=None, error=None, at_time=None, reason="lease finished"):
        ...
        if status in result_statuses:
            ...
            newer = [row for row in self.snapshot.resources.get("leases", [])
    ...
    def admit_semantic_problem(
        ...
        *,
        reason: str = "semantic problem admitted",
    ) -> dict[str, Any]:
        report = validate_problem_instance(domain, definition, model, instance)
        if not report["valid"]:
            raise ValueError(f"semantic problem rejected: {report['errors']}")
        current = self.semantic_problem_report()
        if current.get("configured"):
            current_instance = current["problem_instance"]
            if current_instance.get("instance_id") == instance.instance_id and current_instance.get("fingerprint") == instance.fingerprint:
                return current
        ...
        evidence = EvidenceRecord(
            kind="semantic_problem",
            statement=canonical_semantic_json(document),
            source=SEMANTIC_PROBLEM_CONTRACT_ID,
            metadata={
                "semantic_contract_id": SEMANTIC_PROBLEM_CONTRACT_ID,
                "semantic_fingerprint": instance.fingerprint,
        ...
        stored = self.add_evidence(evidence, reason=reason)
        out = self.semantic_problem_report()
        out["admission_evidence_id"] = stored.evidence_id
        return out
    ...
    def semantic_problem_report(self) -> dict[str, Any]:
        ...
        selected = [row for row in records if row.get("kind") == "semantic_problem" and row.get("status") == "active"
                    and (row.get("metadata") or {}).get("semantic_contract_id") == SEMANTIC_PROBLEM_CONTRACT_ID]
        if not selected:
            return {"contract": semantic_problem_contract(), "configured": False}
        row = selected[-1]
        document = json.loads(row["statement"])
        domain, definition, model, instance = semantic_problem_from_document(document)
        report = validate_problem_instance(domain, definition, model, instance)
        return {
            "contract": semantic_problem_contract(),
            "configured": True,
    ...
    def semantic_domain_report(self) -> dict[str, Any]:
        report = self.semantic_problem_report()
        return {
            "contract": report["contract"],
            "configured": report.get("configured", False),
            "domain_package": report.get("domain_package"),
            "problem_model": report.get("problem_model"),
        }
