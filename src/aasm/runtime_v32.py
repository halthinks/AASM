...
class AASMEngine(ReasoningRuntimeMixin, V31Engine):
    ...
    def _finish_lease(self, lease_id: str, status: str, *, result=None, error=None, at_time=None, reason="lease finished"):
        ...
        if status in {LeaseStatus.COMPLETED.value, LeaseStatus.FAILED.value}:
            if lease.get("status") != LeaseStatus.ACTIVE.value: raise ValueError(f"Lease {lease_id} cannot accept a worker result from status {lease.get('status')}")
            ...
            if newer: raise ValueError(f"Lease {lease_id} is stale; task {lease.get('task_id')} is owned by newer attempt(s) {sorted(row.get('lease_id') for row in newer)}")
    ...
    def admit_semantic_problem(self, domain: DomainPackage, definition: ProblemDefinition, model: ProblemModel, instance: ProblemInstance, *, reason: str = "semantic problem admitted") -> dict[str, Any]:
        ...
        evidence = EvidenceRecord(
            kind="semantic_problem", statement=canonical_semantic_json(document), source=SEMANTIC_PROBLEM_CONTRACT_ID,
            metadata={"semantic_contract_id": SEMANTIC_PROBLEM_CONTRACT_ID, "semantic_fingerprint": instance.fingerprint,
                      "domain_package_id": domain.package_id, "domain_package_fingerprint": domain.fingerprint,
                      "instance_id": instance.instance_id, "scope_id": "root"},
        )
    ...
    def semantic_problem_report(self) -> dict[str, Any]:
        ...
        return {"contract": semantic_problem_contract(), "configured": True, "evidence_id": row["evidence_id"], "valid": report["valid"], "validation": report,
                "domain_package": domain.to_dict(), "problem_definition": definition.to_dict(), "problem_model": model.to_dict(), "problem_instance": instance.to_dict()}
