...
def make_handler(store_target:str,token:str|None=None):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if parsed.path=="/health": return self._json(200,{"ok":True,"protocol":"aasm.remote.v1","version":"0.11.0"})
            if parsed.path=="/ui":
            ...
            try:
                if len(parts)==4 and parts[:2]==["v1","machines"] and parts[3] in {"state","dashboard"}:
                    ...
                    try:
                        ...
                        else: payload={"snapshot":asdict(engine.snapshot),"workers":engine.list_workers(),"leases":engine.list_leases(),"models":engine.list_model_profiles(),"last_model_route":engine.last_model_route(),"model_performance":engine.model_performance()}
                    finally: store.close()
