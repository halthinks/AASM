...
class AASMEngine(V07Engine):
    ...
    def recover_unfinished(cls,store,authority=None,*,recover_effects=False,load_history=True):
        return [
            cls.resume(mid,store,authority=authority,recover_effects=recover_effects,load_history=load_history)
    ...
    def _sync_after_append(self):
        ...
        if fresh:
            ...
            self._last_sequence=fresh[-1].sequence
        ...
        try:
            ...
            try:
                ...
                if self._history_loaded:
                    ...
                    last_loader=getattr(self.store,"last_event_sequence",None)
                    self._last_sequence=last_loader(machine_id) if last_loader else self._last_sequence
                    self.events=[]
                self._refresh_runtime_views()
            except Exception:
                pass
