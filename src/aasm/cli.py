...
def _schedule(args): store=_open(args); e=AASMEngine.resume(args.machine_id,store); tasks=[TaskDemand(**x) for x in json.load(open(args.tasks,encoding="utf-8"))]; _json(e.schedule(tasks).to_dict()); store.close()
...
def _claim(args): store=_open(args); e=AASMEngine.resume(args.machine_id,store); _json(e.claim_task(TaskDemand(**json.load(open(args.task,encoding="utf-8"))),args.worker,lease_seconds=args.lease_seconds)); store.close()
...
def _model_route(args): store=_open(args); e=AASMEngine.resume(args.machine_id,store); _json(e.route_model(ModelRouteRequest(**json.load(open(args.request,encoding="utf-8")))).to_dict()); store.close()
def _model_outcome(args): store=_open(args); e=AASMEngine.resume(args.machine_id,store); _json(e.record_model_outcome(ModelOutcomeRecord(**json.load(open(args.record,encoding="utf-8"))))); store.close()
...
def build_parser():
    ...
    def stored(name,help): q=s.add_parser(name,help=help); return q
    q=stored("demo","run demo"); _add_store_args(q,required=False); q.set_defaults(func=_demo)
    for name,func in [("runs",_runs)]: q=stored(name,name); _add_store_args(q); q.set_defaults(func=func)
    ...
    for name,func in [("effects",_effects),("plan",_plan),("memory",_memory),("resources",_resources),("workers",_workers),("models",_models),("economics",_economics)]: q=stored(name,name); q.add_argument("machine_id"); _add_store_args(q); q.set_defaults(func=func)
