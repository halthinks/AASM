...
| 59.1 | `engineering-semantics-production-search` / S4-01 | Quantity/unit/tolerance semantics | GATED | exact dimensional correctness and portable engineering value identity | `aasm.quantity.v1`; `src/aasm/quantity.py`; strict `quantity.schema.json`; active additive public surface `0.32.16`; `check_quantity_contracts.py`; `check_quantity_public.py` | 55.1; S3 complete | exact head `263640a634da0e92bb1ae0b42cb55063e0b64552`; `aasm/engineering-quantity` + cumulative `aasm/v56` + formal + full CI; exact-number/float rejection, affine conversion, dimension, interval, uncertainty, tolerance, quantization, precision, tamper, equivalence, legacy-substrate non-regression | public semantic IR only; runtime admission `PRE_ADMISSION_ONLY`; no unit registry, FactAuthority/effect authority, postcondition/capability/solver reinterpretation; `aasm.numeric.tolerance.v1` and `EffectCapability.NumericInterval` unchanged | preserve; NEXT 59.2 `aasm.rule.v1` |
| 59.2 | `engineering-semantics-production-search` / S4-02 | Rule applicability/precedence | DESIGNED | hard-floor/hard/policy/preference/advisory semantics with applicability, waiver and revision/source authority | target `aasm.rule.v1`; existing calculus `LearnedConstraint(HARD|SOFT)` and objective hard floors are separate substrates, not the new rule contract | 55.1; 59.1 | scope/revision/applicability/precedence/waiver/source-authority attacks required | objective priority cannot override rule authority; Rule strength must not redefine learned-constraint strength | **NEXT:** pre-admission Rule model/schema/tests/firewall + `aasm/engineering-rule` gate |
...
### S3 artifact/entity + S4 Quantity cumulative qualification
...
All **28 current custom commit-status contexts** were `success` on that exact head, including:
...
The cumulative `aasm/v56` run passed the full inherited solver/provenance/PR-1/PR-2/PR-3/S3 chain, then the S4 Quantity source/public firewalls and both Quantity adversarial/public corpora, cumulative release-contract validation, and the active `0.32.16` authority-boundary guard.
...
Entity Evolution remains an immutable additive `0.32.15` parent layer inherited by the later `0.32.16` Quantity root; advancing the public root does not rewrite the historical entity contract layer.

This evidence qualifies **S3 complete + 59.1/S4-01 Quantity as GATED**, not RELEASED.
...
3. **NEXT: implement pre-admission `aasm.rule.v1`** with stable identity, external/source references, explicit applicability context, scope selector, priority, specificity, `HARD_FLOOR | HARD | POLICY | PREFERENCE | ADVISORY`, waiver/override policy, severity, source authority, and exact problem/external revision applicability.
4. Preserve existing formal-calculus `LearnedConstraint(strength=HARD|SOFT)` as a distinct conflict-learning semantic object. Do not alias source Rule strength to learned-constraint strength; any lowering must be explicit, versioned, provenance-preserving, independently checkable, and fail closed.
5. Add strict portable Rule schema, deterministic fingerprints, adversarial precedence/waiver/revision/scope/source-authority fixtures, source firewall, and independent `aasm/engineering-rule` gate before public or engine admission.
6. Keep Quantity public but runtime-pre-admission. Future Quantity integration with EffectCapability, postconditions, solver/provider tolerance, or physical-effect semantics must use explicit translation/admission contracts; do not silently reinterpret existing substrates.
...
8. Continue applying canonical portable identity/serialization/bounded-integer rules to every S4 object so later Rust does not require contract redesign.
9. Continue S4 projection/equivalence, uncertainty/scenario/trace, degraded-operation, risk/irreversibility, obligation phase, hybrid safety, epistemic debt and manual-override semantics after Rule qualifies.
10. Then implement S5 RefinementLoop/Experiment/VerificationPlan/KnowledgeApplication on existing Evidence/ProblemDelta/authority/resource/dependency planes.
