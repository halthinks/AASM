...
def main():
    ...
    if str(project["version"]) != "0.56.1":
        raise SystemExit(f"unexpected release version: {project['version']}")
    ...
    require(root / "src/aasm/public_v56.py", [
        '__version__ = "0.56.1"', '"contract_version": "0.32.1"',
        'PUBLIC_RELEASE_STABILITY = "ACTIVE_DEVELOPMENT"',
        "SOLVER_OUTCOME_V2_CONTRACT_ID", "SOLVER_RUNTIME_PROVENANCE_CONTRACT_ID",
        "SOLVER_EXECUTION_PROFILE_CONTRACT_ID", "SolverRuntimeProvenance", "solver_provenance_runtime_contract",
        '"interrupted_provenance_v2": "DORMANT_NON_AUTHORITATIVE_NOT_EXPOSED"',
    ])
    require(root / "src/aasm/runtime_v56_foundation.py", ["SolverProvenanceRuntimeMixin", "SolverOutcomeV2RuntimeMixin", "V55FoundationEngine"])
    ...
    require(root / "src/aasm/provider_status_v2.py", [
        ...
        '"fuzzy_matching": "FORBIDDEN"', '"substring_inference": "FORBIDDEN"',
    ])
    require(root / "src/aasm/solver_provenance.py", [
        ...
        '"truth_authority": "NONE"', '"policy_authority": "NONE"',
    ])
    require(root / "src/aasm/_runtime_v56_provenance.py", [
        ...
        '"parallel_provenance_table": "NONE"', '"provenance_grants_reproducibility": False',
        "record_convex_solver_runtime_provenance", "evaluate_solver_runtime_profile",
    ])
    require(root / "src/aasm/solver_execution_observation.py", [
        "aasm.optimization.pysat-cadical", "aasm.optimization.ortools-cp-sat",
        "aasm.optimization.highs", "aasm.optimization.cvxpy",
        "UNAVAILABLE_FROM_CURRENT_ADAPTER", "BACKEND_SPECIFIC_NOT_EXPOSED_BY_CURRENT_CVXPY_ADAPTER",
    ])

    for schema in (
        "solver-outcome-v2.schema.json", "provider-status-map.schema.json",
        "solver-execution-profile.schema.json", "solver-runtime-provenance.schema.json", "solver-profile-evaluation.schema.json",
    ):
        require(root / "schemas" / schema, ['"$schema"', "2020-12"])

    # Frozen semantic-evolution and earlier parents remain intact.
    ...
    require(root / "src/aasm/semantic_evolution.py", [
        ...
        'PROBLEM_REVISION_CONTRACT_ID = "aasm.problem.revision.v1"', 'PROBLEM_DELTA_CONTRACT_ID = "aasm.problem.delta.v1"',
    ])
    ...
    for script in (
        "check_v52_contracts.py", "check_v53_contracts.py", "check_v53_solver_learning_contracts.py", "check_v54_contracts.py",
        "check_v55_discrete_ir.py", "check_v55_scheduling_ir.py", "check_v55_continuous_ir.py",
        "check_v55_decision_vector.py", "check_v55_semantic_archive.py",
        "check_v56_solver_outcome.py", "check_v561_provenance.py",
    ):
        run_script(root, script)

    require(root / "README.md", ["Current release — v0.56.1", "aasm.adoption.v1 / 0.32.1", "v0.56.2"])
    require(root / "docs/CURRENT_RELEASE.md", ["AASM v0.56.1", "0.32.1", "Runtime Provenance", "v0.56.2"])
    require(root / "docs/RELEASE_0.56.1.md", ["AASM v0.56.1", "56.2", "CVXPY", "v0.56.2"])
    require(root / ".github/workflows/v56.yml", ["v0.56.1", "check_v561_provenance.py", "tests/test_v561_solver_provenance_real.py", "context='aasm/v56'"])
    require(root / ".github/workflows/v561.yml", ["AASM v0.56.1 Execution Provenance", "context='aasm/v56-provenance'"])
    require(root / ".github/workflows/release.yml", ["aasm/v54 aasm/v55 aasm/v56 aasm/v56-provenance", "verify-github-release"])

    print("v0.56.1 release contracts: PASS")
    return 0
